...
def exec_formatted_command(args):
    ...
    if os.path.exists(args.clippy_driver):
        ret = subprocess.run([args.clippy_driver, *rustc_args], env=env, check=False)
    else:
        # Fallback: rustc directly. rustc_args[0] is rustc path (for clippy), skip it.
        ret = subprocess.run([args.rustc, *rustc_args[1:]], env=env, check=False)
    if ret.returncode != 0:
